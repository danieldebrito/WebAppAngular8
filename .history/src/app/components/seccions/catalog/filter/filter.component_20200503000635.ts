    ...
    this.cardsService.FiltrarP(
      ...
      this.aplicacion).then(
        response => {
          ...
          const dataFiltradaRet = this.dataFiltrada;

          const tam = this.dataFiltrada.lenght;


          for (let i = 0; i < tam; i++) {
            if (this.dataFiltrada[i].idArticulo === this.dataFiltrada[i + 1].idArticulo
              && this.dataFiltrada[i].motor === this.dataFiltrada[i + 1].motor && i < tam - 1) {
              dataFiltradaRet[i].modelo += dataFiltradaRet[i + 1].modelo;
              dataFiltradaRet.splice(i + 1);
            }
