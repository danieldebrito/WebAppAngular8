...
export class CarritoComponent implements OnInit {
  ...
  public cerrarPedido() {
    this.updateidPedidoCarritoItems(/*this.getPedidoClienteAbierto(this.clienteLogueado.idCliente)*/ 3);
  }
  ...
  public async getCarritoItems() {
    ...
    (await this.carritoItemsService.getCarritoItems()).subscribe(elements => {
      this.carritoItems = elements.filter(item => item.idArticulo === this.clienteLogueado.idCliente && item.idPedido === '-1');
      this.getSubtotal();
    });
  }
