...
export class PedidosComponent implements OnInit {
  ...
  public ListarPedidosCliente() {
    this.pedidosService.Listar().subscribe(pedidos => {
      ...
      this.pedidos.map( item => this.idPedido = item.idPedido);
    });
  }

  public traerPedidoPorID() {
    this.pedidosService.TraerUno(this.idPedido).subscribe( response => {
